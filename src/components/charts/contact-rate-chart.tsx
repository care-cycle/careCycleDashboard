...
export function ContactRateChart({
  ...
  const chartData = React.useMemo(() => {
    ...
    const totals = data.data.reduce(
      (acc: { completed: number; created: number }, point: DataPoint) => ({
    ...
    const windowSize = 30;
    const movingAverages = data.data.map(
          ...
          {summaryMetrics && (
            <div className="flex items-center space-x-6 text-sm">
              <div>
                ...
                <span className="ml-2">
                  {summaryMetrics?.completed?.toLocaleString() || "0"} /{" "}
                  {summaryMetrics?.total?.toLocaleString() || "0"} successful
                </span>
              </div>
              <div>
                <span className="font-medium">Contact Rate:</span>
                <span className="ml-2">
                  {summaryMetrics?.contactRate || "0"}%
                </span>
              </div>
            </div>
          )}
            ...
            <div className="h-[400px]">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Tooltip
                    formatter={(value: number, name: string, props: any) => {
                      ...
                      if (name === "30-Day Per-Dial Rate") {
                        ...
                        result.push(
                          `${dataPoint.uniqueCallers} completed / ${dataPoint.totalCalls} total calls`,
                        );
                      } else {
                        result.push(`${value.toFixed(1)}% contact rate`);
                        result.push(
                          `${dataPoint.uniqueCallers30d} completed / ${dataPoint.totalCustomers30d} total attempts`,
                        );
                      }
                  ...
                  {chartData.map(
                    (entry: ContactRateDataPoint, index: number) => {
            ...
            <div className="mt-4 text-sm text-muted-foreground">
              <div className="flex items-center justify-between">
                ...
                <div>
                  ...
                  30-Day Per-Dial Rate: Success rate of calls over a rolling
                  30-day window
                </div>
              </div>
            </div>
