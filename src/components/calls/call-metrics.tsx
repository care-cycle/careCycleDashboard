...
export function CallMetrics({ calls, date, isLoading }: CallMetricsProps) {
  const calculatedMetrics = useMemo(() => {
    ...
    if (date?.from && date?.to) {
      ...
      daysDiff = Math.ceil((date.to.getTime() - date.from.getTime()) / (1000 * 60 * 60 * 24));
    ...
    const previousAverageDurationMs = previousPeriodCalls.length > 0 
      ? previousTotalDurationMs / previousPeriodCalls.length 
      : 0;

    // Calculate percentage changes
    const callsChange = previousPeriodCalls.length === 0 
      ? null 
      : ((totalCalls - previousPeriodCalls.length) / previousPeriodCalls.length * 100);

    const durationChange = previousAverageDurationMs === 0
      ? null
      : ((averageDurationMs - previousAverageDurationMs) / previousAverageDurationMs * 100);

    return [
      {
        title: "Total Calls",
        value: totalCalls.toLocaleString(),
        change: callsChange === null 
          ? (date?.from ? `No data for previous ${daysDiff} day${daysDiff === 1 ? '' : 's'}` : 'Select a date range for comparison')
          : `${callsChange >= 0 ? '+' : ''}${callsChange.toFixed(1)}%`,
        trend: callsChange === null ? "neutral" : callsChange >= 0 ? "up" : "down"
      },
      {
        title: "Average Duration",
        value: formatDuration(Math.round(averageDurationMs)),
        change: durationChange === null
          ? (date?.from ? `No data for previous ${daysDiff} day${daysDiff === 1 ? '' : 's'}` : 'Select a date range for comparison')
          : `${durationChange >= 0 ? '+' : ''}${durationChange.toFixed(1)}%`,
        trend: durationChange === null ? "neutral" : durationChange >= 0 ? "up" : "down"
      }
    ];
  }, [calls, date, isLoading]);

  if (isLoading) {
    return <div className="grid gap-4 md:grid-cols-2 max-w-[750px]">Loading metrics...</div>;
  }
