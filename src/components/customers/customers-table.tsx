  ...
  const allColumns = useMemo(() => {
    ...
    const columns = [
      ...
      { 
        key: "last-contact", 
        label: "Last Contact", 
        render: (customer: Customer) => customer.lastCallDate ? formatDate(customer.lastCallDate) : '-'
      }
    ];
    ...
    return columns;
  }, [dataFields, renderCustomerName, renderCustomerId, renderContact, navigate, isRedacted]);
        ...
        <Table>
          ...
          <TableBody>
            {customers.length > 0 ? (
              ...
              <TableRow>
                <TableCell colSpan={activeColumns.length + 2} className="text-center py-4">
                  No data available
                </TableCell>
              </TableRow>
            )}
